  ...
  app.post("/webhooks/yookassa", async (request, reply) => {
    ...
    try {
      ...
      return reply.status(502).send({
        success: false,
        error: { code: "PROVIDER_ERROR", message: "Не удалось верифицировать платёж у провайдера" },
      });
    }
    ...
    if (verified.status === "succeeded") {
      ...
      if (transitioned) {
        const [user] = await app.db.select().from(users).where(eq(users.id, order.userId)).limit(1);
